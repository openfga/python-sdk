...
class OpenFgaClient:
    ...
    def batch_check(
        ...
        aliases_by_type = (
            self._get_relation_aliases(options) if optimize_relation_aliases else {}
        )
        ...
        with ThreadPoolExecutor(max_workers=max_parallel_requests) as executor:
            for response in executor.map(single_batch_check, checks):
                for correlation_id, check_result in response.result.items():
    ...
    def list_relations(
        ...
        if any(
            ...
            return self._list_relations_with_individual_checks(body, options)
    ...
    def _list_relations_with_individual_checks(
        self,
        body: ClientListRelationsRequest,
