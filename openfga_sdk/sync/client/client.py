...
from openfga_sdk.client.models.batch_check_item import ClientBatchCheckItem, construct_batch_item
...
class OpenFgaClient:
    ...
    def batch_check(self, body: ClientBatchCheckRequest, options=None):
        """
        ...
        with ThreadPoolExecutor(max_workers=max_parallel_requests) as executor:
            for response in executor.map(single_batch_check, checks):
                result.extend([
                    map_response(c_id, c_result) for c_id, c_result in response.result.items()
                ])
    ...
    def list_users(self, body: ClientListUsersRequest, options: dict[str, str] = None):
        """
