...
from openfga_sdk import __version__ as package_version, Configuration
from openfga_sdk.api_client import DEFAULT_USER_AGENT as ASYNC_DEFAULT_USER_AGENT  # alias in module
...
from openfga_sdk.constants import (
    ...
    MAX_BACKOFF_TIME_IN_SEC, SAMPLE_BASE_DOMAIN,
)


def test_version_constant_matches_package_version():
    assert package_version == SDK_VERSION, "openfga_sdk.__version__ must match SDK_VERSION constant"
...
def test_user_agent_contains_version():
    assert SDK_VERSION in USER_AGENT, "USER_AGENT should embed the SDK version for observability"
