        ...
        response_body = """
{
  "objects": [
    ...
    @patch.object(rest.RESTClientObject, "request")
    async def test_list_relations_optimizes_relation_aliases(self, mock_request):
    ...
    async def test_list_relations_rechecks_aliases_after_batch_error(
        ...
        def mock_error_then_checks(method, url, **kwargs):
            if method == "GET":
                return mock_response(model_response, 200)
            if url.endswith("/batch-check"):
                correlation_id = kwargs["body"]["checks"][0]["correlation_id"]
                return mock_response(
                    json.dumps(
                        {
                            "result": {
            ...
            return mock_response(
                json.dumps({"allowed": relation == "can_add_child"}), 200
            )

        mock_request.side_effect = mock_error_then_checks
        configuration = self.configuration
        configuration.store_id = store_id
