...
def api_request_conditions(
    ...
    return {
        "attribute": "read_authorization_model",
        "new": AsyncMock(return_value=api_request_response),
    }


@pytest.mark.asyncio
class TestOpenFgaClientReadAuthorizationModelEndpoint:
    """
    ...
    async def test_read_authorization_model_issues_request(
        ...
        """
        Test that the read_authorization_model method issues a request to OpenFgaApi.
        """
    ...
    async def test_read_authorization_model_returns_expected_response(
        ...
        """
        Test that the read_authorization_model method returns the expected response.
        """
        with patch.object(client.api, **api_request_conditions):
            response = await client.read_authorization_model(str(uuid.uuid4()))

            assert response is None

    async def test_read_authorization_model_returns_full_response(
        self,
        client: OpenFgaClientProtocol,
        api_request_response: ApiResponse,
        api_request_conditions,
        ...
        """
        Test that the read_authorization_model method returns an ApiResponse instance containing the expected response.
        """
