...
def rest_client_response(
    ...
    api_response = {"authorization_model": mock_api_response_read_authorization_model}
...
def expected_response(
    ...
    response.deserialized = mock_api_response_read_authorization_model_deserialized
    return response
...
@pytest.mark.asyncio
class TestOpenFgaApiReadAuthorizationModelEndpoint:
    """
    @covers openfga_sdk.api.open_fga_api.OpenFgaApi.read_authorization_model.
    """

    async def test_read_authorization_model_issues_request(
        self,
        api: OpenFgaApiProtocol,
        api_client_request_conditions,
    ):
        """
        Test that the read_authorization_model method issues a request to ApiClient.
        """
        with patch.object(
            api.api_client, **api_client_request_conditions
        ) as api_request:
            await api.read_authorization_model(str(uuid.uuid4()))

            api_request.assert_called_once()

    async def test_read_authorization_model_returns_expected_response(
        self,
        api: OpenFgaApiProtocol,
        ...
        """
        Test that the read_authorization_model method returns an ApiClientResponse instance containing the expected response.
        """

        with patch.object(api.api_client, **api_client_request_conditions):
            response = await api.read_authorization_model(str(uuid.uuid4()))
