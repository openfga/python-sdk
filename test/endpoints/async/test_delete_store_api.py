...
@pytest.mark.asyncio
class TestOpenFgaApiDeleteStoreEndpoint:
    """
    @covers openfga_sdk.api.open_fga_api.OpenFgaApi.delete_store.
    """
    ...
    async def test_delete_store_issues_request(
        ...
        """
        Test that the delete_store method issues a request to ApiClient.
        """
        with patch.object(
            api.api_client, **api_client_request_conditions
        ) as api_request:
            await api.delete_store()
    ...
    async def test_delete_store_returns_expected_response(
        ...
        """
        delete_store_body = DeleteStoreRequestOptions(
            store_id=str(uuid.uuid4()),
        )

        with patch.object(api.api_client, **api_client_request_conditions):
            response = await api.delete_store(delete_store_body)
